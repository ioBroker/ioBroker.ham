...
const buildQueryString = (skip, take, search) => `${SEARCH_URL}${!!search ? encodeURIComponent(' ' +search) : ''}&from=${skip}&size=${take}`;
...
const ToolTipButton = ({tooltip, disabled, Icon, ...other}) => {
    return !!disabled
        ? (<Button disabled={true} {...other}><Icon /></Button>)
        : (<Tooltip title={tooltip}><Button {...other}><Icon /></Button></Tooltip>)
};
...
const ActionsFormatter = ({row}) => {
  return (
    <ButtonGroup size="small" aria-label="outlined primary button group">
        ...
        <ToolTipButton tooltip={row.installed ? "Update" : "Install"} disabled={row.installed && row.installed === row.package.version} Icon={row.installed ? Update : GetApp} />
...
const cleanModuleName = (name) => {
    return name.replace('homebridge-', '').replace('-homebridge', '').replace(/^@.+?\//, '');
}

export default (props) => {
  const [state, dispatch] = useReducer(reducer, {...initialState, installed: props.installed && props.installed.filter(p => !!p) || []});
  ...
  const prependInstalled = (results, total, installed, search) => {
    ...
    if (!!search) {
        const searchTerms = search.split(/\s+/).filter(t => !!t).map(t => t.toLowerCase());
        filterSearch = item => !searchTerms.filter(t => !item.package.name.toLowerCase().includes(t) && !item.package.description.toLowerCase().includes(t)).length;
    }

    const versions = {};
    const names = [];
    installed.map(m => m.split('@')).forEach(parts => {
        names.push(parts[0]);
  ...
  const loadData = () => {
    const {
      requestedSkip, take, search, lastQuery, loading, installed
    } = state;
  ...
  const {
    rows, skip, totalCount, loading, search
  } = state;
  return (
      <>
        <SearchBar

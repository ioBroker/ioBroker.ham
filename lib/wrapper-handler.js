...
function init(config) {
    ...
    async function accessoryLogic(accessory, external, isUpdate) {
        async function iterateCharArray(chars, dev_idname, sr_id, sr_idname, accessory) {
        ...
        logger.debug(`iobroker.ham Bridge ${isUpdate ? 'updateBridgedAccessory' : 'addBridgedAccessory'} (ext=${external}) ${accessory.UUID}: ${accessory.displayName}`); //OK
        // New device
        ...
        for (const index in accessory.services) {
            ...
            await iterateCharArray(service.characteristics, dev_idname, sr_id, sr_idname, accessory);
        }
    }

    homebridgeWrapper.on('addAccessory', accessory =>
        accessoryLogic(accessory, false));

    homebridgeWrapper.on('updateAccessory', accessory =>
        accessoryLogic(accessory, false, true));
...
async function setValueForCharId(id, value, callback) {
    if (charMap[id]) {
        ...
        homebridgeWrapper.pollAccessoryService(charMap[id].accessory, service);
    }
    else {

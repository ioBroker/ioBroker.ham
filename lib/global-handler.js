...
function init(config) {
    ...
    if (fs.existsSync(path.join(config.homebridgeBasePath, 'node_modules', 'hap-nodejs'))) {
        hapBasePath = path.join(config.homebridgeBasePath, 'node_modules', 'hap-nodejs');
    }
    else if (fs.existsSync(path.join(config.homebridgeBasePath, '..', 'hap-nodejs'))) {
        hapBasePath = path.join(config.homebridgeBasePath, '..', 'hap-nodejs');
    }
    else {
        try {
            hapBasePath = require.resolve('hap-nodejs', {paths: [path.join(config.homebridgeBasePath, 'node_modules'), config.homebridgeBasePath]});
        }
        catch (err) {
    ...
    once = require(path.join(path.dirname(hapBasePath), 'lib', 'util', 'once')).once;

...
function init(config) {
    ...
    function handleCharacteristicPolling(accessory, service, characteristic) {
        if (characteristicPollingInterval) {
            ...
            characteristicPollingTimeouts[`${accessory.UUID}.${service.UUID}.${characteristic.UUID}`] = setTimeout(function () {
                characteristic.getValue(async function (err, value) {
    ...
    async function iterateCharArray(chars, accessory, service, dev_idname, sr_id, sr_idname) {
        for (const channelIndex in chars) {
            ...
            char.on('change', async data => {
                logger.debug(`Char change event: ${data.oldValue} --> ${data.newValue}`);
                await handleCharValue(accessory, service, char, data.newValue);
            });
    ...
    if (!InterceptedBridge) {
        function MyBridge(displayName, serialNumber) {
            ...
            this.__wrapperAccessoryLogic = async function(accessory, external) {
                ...
                for (const index in accessory.services) {
                    ...
                    if (service.optionalCharacteristics) {
                        await iterateCharArray(service.optionalCharacteristics, accessory, service, dev_idname, sr_id, sr_idname);
                    }
                }
            };
...
function setValueForCharId(id, value, callback) {
    if (charMap[id]) {
        logger.debug(`set value ${JSON.stringify(value)} of char for ${id}`);
        charMap[id].setValue(value, callback);
    }
    else {
        logger.info(`id ${id} not known`);

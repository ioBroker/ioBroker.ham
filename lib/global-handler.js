...
function init(config) {
    ...
    let hapBasePath;
    try {
        hapBasePath = require.resolve('hap-nodejs', {paths: [path.join(config.homebridgeBasePath, 'node_modules').replace(/\\/g, '/'), config.homebridgeBasePath.replace(/\\/g, '/')]});
    }
    catch (err) {
        console.log('hap-nodejs resolve path: ' + JSON.stringify([path.join(config.homebridgeBasePath, 'node_modules').replace(/\\/g, '/'), config.homebridgeBasePath.replace(/\\/g, '/')]))
        hapBasePath = require.resolve('hap-nodejs');

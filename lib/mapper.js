...
module.exports = function (config) {
    ...
    let accTypes;
    if (config.useGlobalHomebridge) {
        accTypes = require(`${config.homebridgeBasePath}node_modules/hap-nodejs/accessories/types`);
    } else {
        accTypes = require('homebridge-plugin-wrapper').HapTypes;
    }

    const forbiddenCharacters = config.forbiddenCharacters
    ...
    module.mapAccessoryUUID = function (accessoryUUID, displayName) {
        ...
        if (displayName) {
            displayName = displayName.replace(forbiddenCharacters, '-').replace(/[\]\[.*,;'"`<>\\\s?]/g, '-');
        }
        else {
        ...
        let found = false;
        for (const uuid in accessoryNameMap) {
            if (accessoryNameMap.hasOwnProperty(uuid) && accessoryNameMap[uuid] === displayName) {
                found = true;
                break;
            }
        }
    ...
    module.mapServiceType = function (serviceUUID, displayName) {
        ...
        if (serviceTypeMap[serviceUUID]) {
            return serviceTypeMap[serviceUUID];
        }

        const pos = serviceUUID.indexOf('-0000-1000-8000-0026BB765291');
        if (pos !== -1) {
            return serviceUUID.substring(0, pos);
        }
    ...
    module.mapCharacteristicType = function (serviceUUID, charUUID, displayName) {
        ...
        if (characteristicTypeMap[charUUID]) {
            return characteristicTypeMap[charUUID].name;
        }

        const pos = charUUID.indexOf('-0000-1000-8000-0026BB765291');
        if (pos !== -1) {
            return charUUID.substring(0, pos);
        }

        return charUUID;
    };

    const characteristicFormats = {
        'bool': 'boolean',
        'int': 'number',
        'float': 'number',
        'string': 'string',
    ...
    module.mapCharacteristicProperties = function (char) {
        ...
        if (char.validValues && char.validValues.length > 0 && common.type !== 'boolean') {
            common.states = {};
            for (let i = 0; i < char.validValues; i++) {
                common.states[char.validValues[i]] = char.validValues[i];
            }
        }

        common.read  = char.props.perms.includes('pr');
        common.write = char.props.perms.includes('pw');
        if (!common.read && !common.write) {
            common.read = true;
        }

        let roleDetail = null;
        if (characteristicTypeMap[char.UUID]) {
            roleDetail = characteristicTypeMap[char.UUID].roleDetail;
        }
        ...
        if (roleDetail && roleDetail.startsWith('.') && role) {
            role += roleDetail;
        } else if (roleDetail && !roleDetail.includes('.')) {
            role = roleDetail;
        }
